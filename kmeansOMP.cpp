...
int main() {
	...
	for (int c = 0; c < NUM_CLUSTERS; c++) {
		...
		p_max = max.data();
		p_med = med.data();
		#pragma omp parallel for reduction(+:p_med[:column]) reduction(min:p_min[:column]) reduction(max:p_max[:column])	
		for (int i = 0; i < n_puntos; i++) {
			for (int j = 0; j < column; j++) {
				indice = clusters[c][i] * column + j;
				if (p_min[j] > all_points[indice]) {
					p_min[j] = all_points[indice];
				}
				if (p_max[j] < all_points[indice]) {
					p_max[j] = all_points[indice];
				}
				p_med[j] += all_points[indice];
			}
		}
